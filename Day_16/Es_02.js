...
let evalPacket = (packet) => {
	switch (packet.id) {
		case 0:
			return packet.value.reduce((prev, next) => {
				return prev + evalPacket(next);
			}, 0);
		case 1:
			return packet.value.reduce((prev, next) => {
				return prev * evalPacket(next);
		...
		case 2:
			return packet.value.slice(1).reduce((prev, next) => {
				return Math.min(prev, evalPacket(next));
			}, evalPacket(packet.value[0]));
		case 3:
			return packet.value.reduce((prev, next) => {
				return Math.max(prev, evalPacket(next));
			}, 0);
		case 4:
			return packet.value;
		case 5:
			return evalPacket(packet.value[0]) > evalPacket(packet.value[1]) ? 1 : 0;
		case 6:
			return evalPacket(packet.value[0]) < evalPacket(packet.value[1]) ? 1 : 0;
		case 7:
			return evalPacket(packet.value[0]) == evalPacket(packet.value[1]) ? 1 : 0;
...
if (packets.length !== 1) console.log("Error");
else console.log(evalPacket(packets[0]));
