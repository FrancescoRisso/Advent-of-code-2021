...
let dimY = Math.max(...dots.map((x) => x[1])) + 1;
	...
	if (fold.direction === "x") {
		grid = grid.map((row) => {
			return row.slice(0, fold.point).map((el, index) => {
				if (el === "#" || (row.length - index - 1 != fold.point && row[row.length - index - 1] === "#")) return "#";
				return ".";
			});
		});
	} else {
		grid = grid.slice(0, fold.point).map((row, rowIndex) => {
			return row.map((el, colIndex) => {
				if (el === "#" || (grid.length - rowIndex - 1 != fold.point && grid[grid.length - rowIndex - 1][colIndex] === "#")) return "#";
				return ".";
			});
		});
	}
